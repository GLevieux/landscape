...
meanLastMean = mean(DTLoc[epoch > max(epoch)-100]$moyenne)
meanLastMin =  mean(DTLoc[epoch > max(epoch)-100]$min)
meanLastMax =  mean(DTLoc[epoch > max(epoch)-100]$max)
...
Evolution du fitness moyen des `r dim(mat)[1]` chromosomes. Les `r nbValFail` valeurs en échec (-inf), soit `r round(nbValFail*100/length(mat))`% des valeurs, sont à NA et donc sorties de la moyenne. Sur les 100 derniers runs, le fitness moyen est de `r meanLastMean`, le fitness max moyen est de `r meanLastMax` et le fitness min moyen de `r meanLastMin`
